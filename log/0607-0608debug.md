...
# June 7 (continued) — Shell page fault, background scripts, yebiao (English)

Session focus: fix **shell startup panic**, make **`. script.sh &`** actually run and show in **`ps` / `yebiao`**, and clarify **environment-variable demo** behaviour. Builds on earlier uaccess / user-libc / trap-return work from the same week.

---

## 1. Symptoms addressed

| Issue | Symptom | Status |
|-------|---------|--------|
| Shell startup panic | After help text: `page fault sepc=… stval=0` in `uart_read_line` | **Fixed** |
| Background script | `. sh01.sh &` prints “task queued” but `ps` only shows kernel + shell | **Fixed** |
| `yebiao` misleading | Raw `state=1`, “never ran user code” for running bg script; duplicate kernel; zombie `sh01.sh` | **Fixed** |
| Demo env isolation | Parent `ooxx=666` visible to bg script after `sleep` | **Not implemented** (global env) |

---

## 2. Shell startup page fault
...
Welcome, root.
Shell commands …
page fault pid=1 sepc=0x80201612 stval=0 cause=15 (unhandled)
panic: page fault
```

`addr2line` → store in `uart_read_line` with **`buf == NULL`**.

### Root cause (likely)

- `cpu_irq_enable()` in `shell_loop` before `uart_prompt_and_read_line` opened a window where timer/trap return could corrupt the stack frame holding the line buffer pointer.
- Nested `uart_getc()` IRQ save/restore inside `uart_read_line` could re-enable interrupts during kernel poll (same class of bug noted in `trap.c` comments).

### Fixes

| File | Change |
|------|--------|
| `boot/uart.c` | NULL check on `buf`; poll with `uart_try_getc()` only (no nested `uart_getc`); call `script_bg_poll()` while waiting for input |
| `usr/console.c` | `line` / `prompt` moved to **static** buffers; `cpu_irq_enable()` moved to **after** line read; `uart_rx_flush()` before each prompt |
| `interrupt/trap.c` | Call `vm_fault_handle` only when `epc` is in **user** range (clearer kernel NULL faults) |

---

## 3. Background shell scripts

### Problem

`script_run_bg()` used `task_create()` + trampoline, but:

1. **`schedule()` is never called** from the timer/soft-IRQ path (disabled after OpenSBI issues) → bg task never runs.
2. No **`proc_alloc()`** → job invisible in `ps`.

### Solution: cooperative kernel bg proc

| File | Change |
|------|--------|
| `usr/script.c` | `script_run_bg()` → `proc_alloc(name, shell_pid)`, return real **pid**; `script_bg_poll()` runs script line-by-line until `sleep` or EOF; `sleep` in bg uses `r_time()` deadline (non-blocking); `script_bg_describe()` for yebiao |
| `boot/uart.c` | `script_bg_poll()` from `uart_read_line` idle loop |
| `usr/console.c` | `[bg] pid N` like ELF bg; call `script_bg_poll()` once after start |
...
ps  →  pid 2  S  sh01.sh
yebiao →  kernel bg script: sleeping (sh01.sh)
```

After script finishes, proc slot is **reaped** (`PROC_UNUSED`), not left as zombie.

### Limitation

Only **one** bg script job at a time (`bg_job` singleton). Second `. sh02.sh &` while first is active is rejected.

---

## 4. `yebiao` improvements

| File | Change |
|------|--------|
| `mem/vm.c` | State labels: `R running` / `S sleeping` / `Z zombie` instead of raw integers |
| `mem/vm.c` | For procs without user PT: show `kernel bg script: running|sleeping (path)` via `script_bg_describe()` |
| `mem/vm.c` | Skip duplicate **pid 0** in `vm_info_all_procs()` loop |

---

## 5. Environment variables (demo gap)

`shell_env` in `usr/shell_env.c` is a **single global table** shared by shell and all bg scripts.

Demo in `重要现场演示项目.txt` expects:

- Parent: `export ooxx=222`, later `ooxx=666`
- Child script: local `ooxx=999`, after `sleep 20` still prints **999**

**Current myos:** parent change during sleep overwrites global `ooxx` → final child `echo` shows parent value (e.g. `8`), not `999`.

**Not fixed today.** Needs per-process or copy-on-bg-start env.
...
| Area | Files |
|------|-------|
| UART / shell | `boot/uart.c`, `usr/console.c` |
| Bg scripts | `usr/script.c`, `include/os.h` |
| Trap | `interrupt/trap.c` |
| Debug | `mem/vm.c` |
...
- Per-process environment for bg scripts (demo item 1)
- Multiple concurrent bg script jobs
- Re-enable preemptive `schedule()` safely for ELF bg (`proc_spawn_exec_bg` still uses dormant `task_create` path)
- Port selected xv6 `usertests` (`copyin` / `copyout`) as regression tests
