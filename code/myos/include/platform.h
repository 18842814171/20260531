...
#else
#define KERNEL_LOAD_ADDR 0x80000000
#endif
...
#ifdef CONFIG_OPENSBI
/* S-mode PLIC context per hart (context 1, 3, …) */
#define PLIC_CTX(hart) ((hart) * 2 + 1)
#else
#define PLIC_CTX(hart) (hart)
#endif

#define PLIC_ENABLE(ctx, id) (PLIC_BASE + 0x2000 + (ctx) * 0x80 + ((id) / 32) * 4)
#define PLIC_THRESHOLD(ctx) (PLIC_BASE + 0x200000 + (ctx) * 0x1000)
#define PLIC_CLAIM(ctx) (PLIC_BASE + 0x200004 + (ctx) * 0x1000)

/* legacy names used in plic.c */
#define PLIC_MENABLE(hart, id) PLIC_ENABLE(PLIC_CTX(hart), id)
#define PLIC_MTHRESHOLD(hart) PLIC_THRESHOLD(PLIC_CTX(hart))
#define PLIC_MCLAIM(hart) PLIC_CLAIM(PLIC_CTX(hart))
#define PLIC_MCOMPLETE(hart) PLIC_CLAIM(PLIC_CTX(hart))
